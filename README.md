...
3. Copy the **secret key** (same page, "Secret keys") into `SUPABASE_SECRET_KEY`.
   Never expose this key to the browser — it's used server-only for the Paystack
   webhook and admin actions.
...
6. **Create your admin account**: sign up normally through `/signup`, then in the
   SQL Editor run:
   ```sql
   update customer set is_admin = true where id = '<your-auth-user-uuid>';
   ```
   (Find the UUID under Authentication → Users.) The admin panel lives at `/admin`.
