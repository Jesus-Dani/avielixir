...
with p as (
  ...
  select id, 'Rose Noir', 'rose-noir', 'Rose, dark berries, musk', 'Spray on pulse points after showering for longest wear.', 48000, true
  from category where slug = 'perfume'
  returning id
)
insert into product_variant (product_id, size_label, price, stock_quantity)
select id, size_label, price, stock from p, (values ('39ml', null::numeric, 25), ('50ml', 58000, 15)) as v(size_label, price, stock);

with p as (
  insert into product (category_id, name, slug, scent_notes, usage_instructions, base_price, is_featured)
  select id, 'Lilac Amber', 'lilac-amber', 'Lilac, amber, sandalwood', 'Apply to wrists and neck; reapply midday for lasting scent.', 48000, true
  from category where slug = 'perfume'
  returning id
)
insert into product_variant (product_id, size_label, price, stock_quantity)
select id, size_label, price, stock from p, (values ('39ml', null::numeric, 20), ('50ml', 58000, 10)) as v(size_label, price, stock);
...
insert into product_variant (product_id, size_label, price, stock_quantity)
select id, size_label, price, stock from p, (values ('39ml', null::numeric, 0), ('50ml', 62000, 8)) as v(size_label, price, stock);

with p as (
  insert into product (category_id, name, slug, scent_notes, usage_instructions, base_price, is_featured)
  select id, 'Velvet Bloom', 'velvet-bloom', 'Peony, vanilla, soft musk', 'Spray from a distance of 15cm onto skin or clothing.', 46000, true
  from category where slug = 'perfume'
  returning id
)
insert into product_variant (product_id, size_label, price, stock_quantity)
select id, size_label, price, stock from p, (values ('39ml', null::numeric, 30), ('50ml', 56000, 12)) as v(size_label, price, stock);

with p as (
  insert into product (category_id, name, slug, scent_notes, usage_instructions, base_price, is_featured)
  select id, 'Citrus Mist', 'citrus-mist', 'Bergamot, grapefruit, white tea', 'Shake well, mist over body from arm''s length.', 15000, false
  from category where slug = 'body-mist'
  returning id
)
insert into product_variant (product_id, size_label, price, stock_quantity)
select id, size_label, price, stock from p, (values ('100ml', null::numeric, 40)) as v(size_label, price, stock);

with p as (
  insert into product (category_id, name, slug, scent_notes, usage_instructions, base_price, is_featured)
  select id, 'Jasmine Roll', 'jasmine-roll', 'Jasmine, white musk', 'Roll onto wrists and neck as needed throughout the day.', 9000, false
